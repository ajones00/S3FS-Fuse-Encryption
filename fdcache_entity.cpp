...
int file_encrypt_decrypt(int fd) {
    RC4_KEY key;
    ...
    lseek(fd, 0, SEEK_SET);
    userKey = (unsigned char *)calloc(sizeof(KEY), 1);
    strcpy((char *)userKey, KEY);
    
    read(fd, &saltedHeader, 16);
    lseek(fd, 0, SEEK_SET);
    
    int fd2 = open("/home/anthony/s3fs-fuse/src/tmp_storage.txt", O_CREAT | O_RDWR | O_TRUNC,S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
    
    //check if the file is salted by seeing if "Salted__" is in the header of the file
    if (strstr((const char *)saltedHeader,salted_) != NULL) {
        //decrypt
        offset -= 16;
        
        //move marker past the salt so reading can start properly
        ...
        //set marker back to beginning of files
        lseek(fd2, 0, SEEK_SET);
        lseek(fd, 0, SEEK_SET);
        
        while ((numRead = read(fd2, &buff, BUFF_SIZE)) > 0){
            write(fd, buff, numRead);
        }
        
        //removes last 16 bytes of file which accounts for salt
        ftruncate(fd, offset);
    ...
    remove("/home/anthony/s3fs-fuse/src/tmp_storage.txt");
    free(outputbuff);
    return 1;
